  ...
  return (
    <div className='flex flex-col gap-2 px-6 py-4'>
      ...
      {project.stack && (
        <div className='flex flex-wrap gap-2'>
          {project.stack.map((tech) => (
            <span
              key={tech}
              className='bg-blue-900 text-blue-100 text-sm font-normal px-3 py-1 rounded-full'
            >
              {tech}
            </span>
          ))}
      ...
      {project.video ? (
        ...
        <ImageCarousel images={project.images} imagesDir={project.imagesDir} />
      ) : null}

      <div className='flex flex-wrap gap-3'>
        {project.github && (
          <SocialButton link={project.github} Icon={FaGithub} text='github' />
        )}
        {project.link && (
          <SocialButton
            link={project.link}
