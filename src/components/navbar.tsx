...
const Navbar = () => {
    ...
    return (
        <nav className="max-w-screen-lg w-screen px-2 py-2 md:py-4">
            {isMobile ? (
                <div className="flex items-center justify-between px-10 py-2 bg-gradient-to-r from-blue-700 to-blue-900
               text-white text-2xl rounded-full shadow-xl"
                >
                    <Link className={getNavItemClass('/')} to="/"><FaHouse/></Link>
                    <Link className={getNavItemClass('/projects')} to="/projects"><FaFolder/></Link>
                    <Link className={getNavItemClass('/resume')} to="/resume"><FaFileLines/></Link>
                    <Link className={getNavItemClass('/about')} to="/about"><FaUser/></Link>
                    <Link className={getNavItemClass('/contact')} to="/contact"><FaEnvelope/></Link>
                </div>
            ) : (
